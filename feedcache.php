...
Plugin URI: http://www.craigjolicoeur.com/blog/feedcache
Description: Caches RSS Feeds for display on your Wordpress site.  This prevents multiple HTTP requests with each page load since the feeds can be read from the cache file.
...
// Constants
define(FEEDCACHE_WIDGET_ID, 'feedcache_widget');
...
add_action('plugins_loaded', 'feedcache_widget_init');
...
// Functions
function feedcache_display_feeds($rss_group = '1') {
	global $wpdb;
	$feed_data = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "feedcache_data WHERE group_id = $rss_group");
	return $feed_data->data;
}

function fc_build_config_file($config_settings) {
		$fpath = FEEDCACHE_FILES_PATH . "feedcache-config.yml";
		$yaml = Spyc::YAMLDump($config_settings);
...
function feedcache() {
    ...
    add_submenu_page('options-general.php', 'FeedCache Options', 'FeedCache', 'switch_themes', __FILE__, 'feedcache_subpanel');
	}
}

function feedcache_widget_init() {
  wp_register_sidebar_widget(FEEDCACHE_WIDGET_ID, __('FeedCache'), 'feedcache_widget');
  wp_register_widget_control(FEEDCACHE_WIDGET_ID, __('FeedCache'), 'feedcache_widget_control');
}

function feedcache_widget($args) {
  extract($args, EXTR_SKIP);
  $options = get_option(FEEDCACHE_WIDGET_ID);  
  $group = $options["group_id"];
  
  echo $before_widget;
  echo feedcache_display_feeds($group);
  echo $after_widget;
}

function feedcache_widget_control() {
  $options = get_option(FEEDCACHE_WIDGET_ID);
  if (!is_array($options)) { $options = array(); }
  
  $widget_data = $_POST[FEEDCACHE_WIDGET_ID];
  if ($widget_data['submit']) {
    $options['group_id'] = $widget_data['group_id'];
    update_option(FEEDCACHE_WIDGET_ID, $options);
  }
  
  // get current number of groups from plugin settings
  $plugin_options = get_option('plugin_feedcache_options');
  $num_of_groups = $plugin_options['group_num'];
  
  // Render the widget form
  $group_id = $options['group_id'];
?>
  <p>
    <label for="<?php echo FEEDCACHE_WIDGET_ID;?>-group-id">RSS Group to display</label>
    <select name="<?php echo FEEDCACHE_WIDGET_ID; ?>[group_id]" id="<?php FEEDCACHE_WIDGET_ID; ?>-group-id">
<?php
      for($i=1; $i<=$num_of_groups; $i++) {
        $selected = ( $group_id == $i ) ? 'selected' : '';
        echo "<option value=\"$i\" $selected>$i</option";
      }
?>
    </select>
  </p>
  <input type="hidden" name="<?php echo FEEDCACHE_WIDGET_ID; ?>[submit]" value="1" />
<?php
}
...
function feedcache_subpanel() {
  ...
  if ($_POST['stage'] == 'process' ) {
		for ($i=1; $i<=$number_of_groups; $i++) {
			$group_array["group$i"] = $_POST["feedcache_rss_list$i"];
		}
		...
		$config_settings = array_merge( $group_array, $new_options );
		$config_settings["db_prefix"] = $wpdb->prefix;
		$config_settings["db_host"] = DB_HOST;
		$config_settings["db_name"] = DB_NAME;
		$config_settings["db_user"] = DB_USER;
		$config_settings["db_password"] = DB_PASSWORD;
		fc_build_config_file( $config_settings );
  }
